  ...
  \item Applied Qual 2022 Problem A
    \begin{enumerate}[label=(\alph*)]
      ...
      \item Using second model, likelihood is just product of LogNormal pdfs
        ...
        &= \prod_{i=1}^{n} \frac{1}{60}\Bigg(\text{LogNormal}(\alpha + \beta t_i)\cdot  \pi_1 dt + \int_{x_i - 3}^{x_i + 3} \text{LogNormal}(\alpha + \beta t_i) \cdot \pi_2 dt \\
    ...
    \item Using second model, do MLE directly. Note the MLEs for LogNormal regression is equivalent to the MLEs for OLS, except with the $y_i$'s replaced with $\log y_i$'s.
      ...
      Using the third model, we could do EM/MCMC/VI, for simplicity we'll just describe an EM algorithm for this model.
  \end{enumerate}
\end{itemize}
